...
class StableVideoDiffusionInpaintingPipeline(DiffusionPipeline):
    ...
    def _encode_image(
        ...
        image = image * 2.0 - 1.0
        image = _resize_with_antialiasing(image, (224, 224)).clamp(-1, 1)
        image = (image * 0.5 + 0.5).clamp(0, 1)

        # Without normalize
        # image = _resize_with_antialiasing(image, (224, 224)).clamp(0, 1)
    ...
    def __call__(
        ...
        export_to_video(frames, "generated.mp4", fps=16)
        ```
        """
