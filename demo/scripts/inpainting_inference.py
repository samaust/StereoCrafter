...
def main(
    ...
    frames = torch.cat([frames_warped, frames_left, frames_mask], dim=0)
    ...
    frames_sbs = (
        (frames_sbs * 255).permute(0, 2, 3, 1).to(dtype=torch.uint8).cpu().numpy()
    )
    write_video_opencv(frames_sbs, fps, frames_sbs_path)

    vid_left = (
        (frames_left * 255).permute(0, 2, 3, 1).to(dtype=torch.uint8).cpu().numpy()
    )
    vid_right = (
        (frames_output * 255).permute(0, 2, 3, 1).to(dtype=torch.uint8).cpu().numpy()
    )
